...
// init the EZPDB structure, and download pdb file from server if necessary
DWORD EzInitPdb(OUT PEZPDB Pdb, IN LPCSTR szModulePath, IN LPCSTR szModuleName, IN BOOL Download, IN OPTIONAL LPCSTR szSymbolServerUrl,  IN OPTIONAL LPCSTR szPdbDownloadDirectory)
{
	strcpy(Pdb->szModulePath, szModulePath);
	strcpy(Pdb->szModuleName, szModuleName);
	...
	Pdb->ReDownload = Download;

	strcpy(Pdb->szDllFile, Pdb->szModulePath);
	if (Pdb->szDllFile[strlen(Pdb->szDllFile) - 1] != '\\')
	{
		strcat(Pdb->szDllFile, "\\");
	}
	strcat(Pdb->szDllFile, Pdb->szModuleName);

	// read pe file to memory, we need debug info directory
	PBYTE FileBuffer = NULL;
	SIZE_T nByte = EzFileToMemory(Pdb->szDllFile, &FileBuffer);
	if (nByte == 0)
	{
